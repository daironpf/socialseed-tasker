...
* [x] **Constraints Configuration System:** Config file (`tasker.constraints.yml`) generated by `tasker init` with:
    * Architecture, technology, naming, patterns, and dependencies constraint categories
    * Hard/soft enforcement levels (hard blocks writes, soft requires agent confirmation)
    * `POST /api/v1/constraints` and `GET /api/v1/constraints` API endpoints
    * `tasker constraints set|list|validate` CLI commands
    * Neo4j storage for active constraints as graph nodes
    * ConstraintEnforcementService in domain layer with Hexagonal Architecture
