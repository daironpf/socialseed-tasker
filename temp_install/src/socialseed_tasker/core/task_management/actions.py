...
def close_issue_action(
    ...
    open_deps = [str(dep.id) for dep in repository.get_dependencies(issue_id) if dep.status != IssueStatus.CLOSED]
    if open_deps:
...
def get_blocked_issues_action(
    ...
    for issue in all_issues:
        if issue.status == IssueStatus.CLOSED:
            continue
        deps = repository.get_dependencies(str(issue.id))
...
def get_dependency_chain_action(
    ...
    visited: set[str] = set()
    chain: list[Issue] = []
    queue: deque[str] = deque([issue_id])
    ...
    while queue:
        ...
        if current_id != issue_id:
            dep_issue = repository.get_issue(str(current_id))
            if dep_issue is not None:
                chain.append(dep_issue)

        for dep in repository.get_dependencies(str(current_id)):
            dep_key = str(dep.id)
...
def _would_create_cycle(
    ...
    while queue:
        ...
        for dep in repository.get_dependencies(str(current)):
            dep_key = str(dep.id)
            if dep_key not in visited:
                queue.append(dep_key)
