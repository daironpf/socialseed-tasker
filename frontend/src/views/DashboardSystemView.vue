  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-gray-500">Project metrics, health checks, and system status</p>
      </div>
    ...
    <template v-else>
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-800">
          <div class="flex items-center justify-between">
            <div>
              ...
              <div class="text-sm text-gray-500">Blocked Issues</div>
            </div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-800">
          <div class="flex items-center justify-between">
            <div>
              ...
              <div class="text-sm text-gray-500">Agents Working</div>
            </div>
      ...
      <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div class="rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800">
          <div class="border-b border-gray-200 px-5 py-4 dark:border-gray-700">
            <h2 class="font-semibold text-gray-900 dark:text-white">System Health</h2>
          </div>
          <div class="p-5 space-y-4">
            ...
            <div class="flex items-center justify-between rounded-lg border border-gray-200 p-4 dark:border-gray-700">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <div class="font-medium text-gray-900 dark:text-white">Neo4j Database</div>
                  <div class="text-xs text-gray-500">Graph database</div>
                </div>
              </div>
            ...
            <div class="flex items-center justify-between rounded-lg border border-gray-200 p-4 dark:border-gray-700">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <div class="font-medium text-gray-900 dark:text-white">Workers</div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800">
          <div class="border-b border-gray-200 px-5 py-4 dark:border-gray-700">
            <div class="flex items-center justify-between">
              <h2 class="font-semibold text-gray-900 dark:text-white">Offline Sync Queue</h2>
              <div class="flex items-center gap-2">
          ...
          <div class="p-5">
            <div v-if="!syncQueue" class="text-center text-sm text-gray-400">Loading sync queue...</div>
            <div v-else-if="syncQueue.queue.length === 0" class="rounded-lg border border-dashed border-gray-300 dark:border-gray-600 p-8 text-center">
              <div class="text-sm text-gray-500">No pending sync items</div>
              <div class="mt-1 text-xs text-gray-400">All data is synchronized with GitHub</div>
            </div>
      ...
      <div class="rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800">
        <div class="border-b border-gray-200 px-5 py-4 dark:border-gray-700">
          <h2 class="font-semibold text-gray-900 dark:text-white">Constraints Summary</h2>
        </div>
        <div class="p-5">
      ...
      <div class="rounded-xl border border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-900/10">
        <div class="border-b border-amber-200 px-5 py-4 dark:border-amber-800">
          <div class="flex items-center gap-2">
            ...
            <h2 class="font-semibold text-amber-800 dark:text-amber-200">Administration</h2>
          </div>
          <p class="mt-1 text-sm text-amber-600 dark:text-amber-400">Protected actions — use with caution</p>
        </div>
        <div class="p-5">
          <div class="flex flex-wrap gap-3">
            <button
              ...
              @click="handleSeed"
            >
              {{ adminLoading ? 'Loading...' : 'Seed Test Data' }}
            </button>
            <button
              :disabled="adminLoading"
              ...
              @click="handleReset"
            >
              {{ adminLoading ? 'Loading...' : 'Reset All Data' }}
            </button>
          </div>
...
async function handleSeed() {
  if (!confirm('Load seed test data? This will overwrite existing data.')) return
  adminLoading.value = true
  adminMessage.value = ''
  try {
    const result = await adminSeed('full', false)
    adminMessage.value = result?.message || 'Seed data loaded successfully'
  } catch (e) {
    adminMessage.value = 'Error: Failed to load seed data'
  } finally {
...
async function handleReset() {
  ...
  try {
    ...
    adminMessage.value = result?.message || 'All data reset'
  } catch (e) {
    adminMessage.value = 'Error: Failed to reset data'
  } finally {
    adminLoading.value = false
  }
}
